...
from dancing.AdaCoF_4_2 import models # model import 
from trainer import Trainer # trainer에서 Trainer 함수 import
...
# 먼저 경로를 여기 폴더로 바꾸어준다.
os.chdir('/home/work/capstone/Final')
path = os.getcwd()
print(f'현재 경로 : --> {path}')
...
# parameters
# Model Selection
parser.add_argument('--model', type=str, default='model') # 여기서 모델 디렉토리에서 어떤 걸 불러올지 정의.
...
parser.add_argument('--load', type=str, default='./dancing/AdaCoF_4_2/model50.pth')
parser.add_argument('--model_path', type=str, default='dancing.AdaCoF_4_2.models.')
...
def main():
    ...
    model = models.Model(args, args.model_path) # adcofnet.py의 AdaCoFNet()클래스 생성
