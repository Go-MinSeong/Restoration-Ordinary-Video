...
class KernelEstimation(torch.nn.Module):
    ...
    def forward(self, rfield0, rfield2):
        ...
        # flow feature map 생성
        tensorflow1 = tensorConv1_ne - tensorConv1_pr
        tensorFlowConv1 = self.flowconv1(tensorflow1)

        # flow와 각 시점에서의 feature map들을 concat -> conv와 pooling 된 것들을 따로 합쳐주어야함 -> Decoder 단에서 Deconvolution하기 때문.
        tensorConv1_pr = torch.cat([tensorConv1_pr,tensorFlowConv1],1)
        tensorConv1_ne = torch.cat([tensorConv1_ne,tensorFlowConv1],1)
        ...
        # flow feature map 생성
        tensorflow2 = tensorConv2_ne - tensorConv2_pr
        tensorFlowConv2 = self.flowconv2(tensorflow2)

        # flow와 각 시점에서의 feature map들을 concat
        tensorConv2_pr = torch.cat([tensorConv2_pr,tensorFlowConv2],1)
        tensorConv2_ne = torch.cat([tensorConv2_ne,tensorFlowConv2],1)
        ...
        # flow feature map 생성
        tensorflow3 = tensorConv3_ne - tensorConv3_pr
        tensorFlowConv3 = self.flowconv3(tensorflow3)

        # flow와 각 시점에서의 feature map들을 concat
        tensorConv3_pr = torch.cat([tensorConv3_pr,tensorFlowConv3],1)
        tensorConv3_ne = torch.cat([tensorConv3_ne,tensorFlowConv3],1)
        ...
        # flow feature map 생성
        tensorflow4 = tensorConv4_ne - tensorConv4_pr
        tensorFlowConv4 = self.flowconv4(tensorflow4)

        # flow와 각 시점에서의 feature map들을 concat
        tensorConv4_pr = torch.cat([tensorConv4_pr,tensorFlowConv4],1)
        tensorConv4_ne = torch.cat([tensorConv4_ne,tensorFlowConv4],1)
        ...
        tensorflow5 = tensorConv5_ne - tensorConv5_pr
        tensorFlowConv5 = self.flowconv5(tensorflow5)

        # flow와 각 시점에서의 feature map들을 concat
        # Conv와 Pool 둘다 생성해주어야 함.
        tensorConv5_pr = torch.cat([tensorConv5_pr,tensorFlowConv5],1)
        tensorConv5_ne = torch.cat([tensorConv5_ne,tensorFlowConv5],1)
