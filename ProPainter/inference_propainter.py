...
def load_images_from_folder(frame_root, sorted_files):
    images = []
    for filename in sorted_files:
        if filename.endswith(".png"):
            image_path = os.path.join(frame_root, filename)
            img = Image.open(image_path)

            left, top, right, bottom = 8, 8, img.width - 8, img.height - 8
...
def sort_files_by_number(folder_path, duration):
    files = os.listdir(folder_path)
    sorted_files = sorted(files, key=lambda x: [int(x.split("_")[1].split(".")[0])])
    if int(duration[1]) >= len(sorted_files):
        duration[1] =  len(sorted_files)
    return sorted_files[int(duration[0]):int(duration[1])]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-o', '--output', type=str, default='/home/kms990321/DiffBIR/project/data/frame_paint', help='Output folder. Default: results')
    parser.add_argument(
    ...
    parser.add_argument(
        '--mode', default='video_outpainting', choices=['video_inpainting', 'video_outpainting'], help="Modes: video_inpainting / video_outpainting")
    parser.add_argument(
        '--scale_h', type=float, default=15/14, help='Outpainting scale of height for video_outpainting mode.')
    parser.add_argument(
        '--scale_w', type=float, default=15/14, help='Outpainting scale of width for video_outpainting mode.')
    parser.add_argument(
        '--save_fps', type=int, default=30, help='Frame per second. Default: 24')
    parser.add_argument(
        '--save_frames', action='store_true', default= True, help='Save output frames. Default: False')
    parser.add_argument(
    ...
    frames, fps, size, video_name = read_frame_from_videos(args.video, args.duration)
    ...
    if args.save_frames:
        for idx in range(video_length):
            ...
            img_save_root = os.path.join(save_root, 'frames', str(idx+args.duration[0]).zfill(4)+'.png')
