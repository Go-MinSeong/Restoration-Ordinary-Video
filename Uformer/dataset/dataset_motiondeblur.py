...
class DataLoaderVal_deblur(Dataset):
    def __init__(self, rgb_dir, img_options=None, rgb_dir2=None):
        ...
        self.inp_filenames = [os.path.join(rgb_dir, x)  for x in inp_files if is_png_file(x)]
        #self.tar_filenames = [os.path.join(rgb_dir, 'groundtruth', x) for x in tar_files if is_png_file(x)]

        self.img_options = img_options
        self.tar_size       = len(self.inp_filenames)  # get the size of target
    ...
    def __getitem__(self, index):
        ...
        return inp_img, inp_img, filename
