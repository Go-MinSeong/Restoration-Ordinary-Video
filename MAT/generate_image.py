...
pyspng = False
def create_array(shape=(1, 512, 512), fill_value=1):
    # 지정된 모양과 0으로 초기화된 NumPy 배열 생성
    array = np.full(shape, fill_value)
    return array

def fill_border_with_ones(array, border_size=20):
    # 바깥쪽 네 방향에 20픽셀 크기의 영역에 1로 채우기
    array[:, :border_size, :] = 0  # 상단
    array[:, -border_size:, :] = 0  # 하단
    array[:, :, :border_size] = 0  # 왼쪽
    array[:, :, -border_size:] = 0  # 오른쪽
...
# CUDA_VISIBLE_DEVICES=1 python generate_image.py --network /home/kms990321/DiffBIR/project/MAT/pretrained/Places_512_FullData.pkl --dpath /home/kms990321/DiffBIR/project/frames512 --outdir /home/kms990321/DiffBIR/project/frames_inpaint