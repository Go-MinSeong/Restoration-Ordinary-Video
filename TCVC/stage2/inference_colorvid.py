...
def get_args_parser():
    ...
    parser.add_argument('--test_path', type=str,default='/home/kms990321/DiffBIR/project/data/frames_color_before')  #/dataset/videvo/test/imgs /dataset/DAVIS/Test/1/imgs

    parser.add_argument('--test_output_path', type=str,default='./stage2_test_results')
    parser.add_argument('--device', default='cuda',
                        help='device to use for training / testing')

    parser.add_argument("--img_size", type=int, default=[360 , 360] )
    parser.add_argument("--nhead_warp", type=int, default=2 )
    parser.add_argument('--num_frames', default=6, type=int,
                        help="Number of frames")
    parser.add_argument('--scale_size', default=[360,360], type=float)
...
def main_testonly_parallel(args):
    ...
    checkpoint_vctran = torch.load(args.model_path,map_location='cuda')
    ...
    checkpoint_decoder = torch.load(args.decoder_path, map_location='cuda')
    ...
    for index,subdir in enumerate(tqdm(subdirs)):
        ...
        for i in tqdm(range(0,len(imgs),1)):
            ...
            Clip.append(transform(img).unsqueeze(0).cuda())
        Clip = torch.cat(Clip,dim=0)
        Clip.requires_grad =False

        tail_flag = 0
        for j in tqdm(range(0,len(Clip),test_num_frames-1)):
            if args.ref_path:
                ref = reference_imgs[j]
